...
import com.agenew.nb.continuouscamera.base.ICameraAction;
import com.agenew.nb.continuouscamera.base.RecordFunc;
...
import com.agenew.nb.continuouscamera.commom.MyToast;
import com.agenew.nb.continuouscamera.commom.Singleton;
import com.agenew.nb.continuouscamera.commom.WeakHandler;
import com.agenew.nb.continuouscamera.state.StateDied;
import com.agenew.nb.continuouscamera.state.StatePicAndPreview;
import com.agenew.nb.continuouscamera.state.StatePicAndRecAndPreview;
import com.agenew.nb.continuouscamera.state.StatePreview;
...
public class MyCameraManager implements WeakHandler.WeakCallback, ICameraAction {
    ...
    private final static String TAG = "MyCameraManager";
    ...
    private void openCameraFirst(Message msg) {
        CamLog.d(TAG, "Goto preview mode error cause it's deed");
    ...
    public void onHandler(Message msg) {
        switch (msg.what) {
            case OPEN: {
                if (mCurrentSt != null) {
                    CamLog.e(TAG, "Error if statemachine is not null!");
                    return;
                }
                ...
                try {
                    mCurrentSt.createSession(new StatePicAndPreview.StateTakePicCb() {
                        ...
                        public void onToken(String path) {
                            CamLog.d(TAG, "onToken in path" + path);
                        }

                        @Override
                        public void onPreviewSuc() {
                            CamLog.d(TAG,"onPreviewSuc in myacmera");
                        }

                        @Override
                        public void onPreviewErr() {
                            CamLog.d(TAG,"onPreviewErr in myacmera");
                        }
                    });
                } catch (Exception e) {
                    CamLog.e(TAG,"start preview err0");
                    e.printStackTrace();
                }
            }
            break;
            ...
            case START_REC: {
                ...
                try {
                    mCurrentSt.createSession(new StatePicAndRecAndPreview.StatePPRCB() {
                        ...
                        @Override
                        public void onToken(String path) {
                            CamLog.d(TAG,"rec:onToken in path" + path);
                        }

                        @Override
                        public void onPreviewSuc() {
                            CamLog.d(TAG,"rec:onPreviewSuc in myacmera");
                        }

                        @Override
                        public void onPreviewErr() {
                            CamLog.d(TAG,"rec:onPreviewErr in myacmera");
                        }
                    });
                } catch (Exception e) {
                    CamLog.e(TAG,"rec:start preview err0");
                    e.printStackTrace();
                }
            }
            break;
