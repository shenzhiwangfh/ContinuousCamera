...
public class StatePreview extends StateBase {
    ...
    private final static String TAG = "StatePreview";
    ...
    protected final Size setSize(int wishWidth, int wishHeight) {
        StreamConfigurationMap map = MyCameraManager.me.getInstance().getCameraCharacteristics().get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
        ...
        CamLog.d(TAG, "after wish size " + needSize);

        if (needSize == null) {
            throw new RuntimeException("No need Camera Size!");
        }

        MyCameraManager.me.getInstance().setPreviewSize(needSize.getWidth(), needSize.getHeight());
        return needSize;
    }
    ...
    protected CameraCaptureSession.StateCallback createStateCallback() {
        return new CameraCaptureSession.StateCallback() {
            ...
            public void onConfigured(CameraCaptureSession session) {
                ...
                try {
                    camera.camSession.setRepeatingRequest(camera.previewBuilder.build(), null, MyCameraManager.me.getInstance().getHandler());
            ...
            public void onConfigureFailed(CameraCaptureSession session) {
                CamLog.e(TAG, "Error Configure Preview!");
                if (mStateBaseCb != null) {
                    StatePreviewCB cb = (StatePreviewCB) mStateBaseCb;
                    cb.onPreviewErr();
                }
